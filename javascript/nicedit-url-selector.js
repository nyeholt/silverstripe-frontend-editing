...
(function ($) {
	...
	SSFrontendEditor.Instance.registerPlugin({
		...
		load: function (editors) {
			var ssauLinkOptions = {
				buttons : {
					'insertlink' : {name : 'Insert Link', type : 'SSFrontendEditor.ssauLinkButton', tags : ['A']}
				}
			};

			SSFrontendEditor.ssauLinkButton = nicEditorAdvancedButton.extend({
				addPane : function() {
					var $this = this;
			...
			editors.registerPlugin(nicPlugin,ssauLinkOptions);
		}
	});
})(jQuery);